...
使用率が高いこと自体は警告にしない。CPU / GPU の使用率 100% はビルド中やゲーム中なら正常、
ディスク Busy 100% は大きなコピー中なら正常、物理メモリの使用率が高いのは Windows がキャッシュ
で空きを埋める正常な挙動である。ここに閾値を置くと常時どこかが点灯し続け、色が警告として
機能しなくなる。警告するのは「枯渇」「温度」「障害」の3カテゴリ、つまりユーザーが実際に
手を打つべき状態だけに絞った。
