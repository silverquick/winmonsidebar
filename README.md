...
| 観測警告 | ディスク高 Busy 継続 | 95%以上が5秒継続<sup>※4</sup> | 99%以上が15秒継続<sup>※4</sup> |
...
同じ色に変える（例: ストレージ一覧の使用率や温度の数字）。警告中の物理ディスク行は行全体を
警告色の枠線（`BorderThickness`固定＋`BorderBrush`差し替え、レイアウトサイズは不変）で囲み、
発報の瞬間だけ枠線を数回パルスさせて注意を引く。`DropShadowEffect` によるグロー効果や行背景の
全面塗りつぶしも試したが、環境（GPU/RDP等）によってはほぼ見えず実機で「気づけない」との
指摘が続いたため不採用にした。
